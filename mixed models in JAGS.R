...
intercept_effects <-
  rnorm(n = n_subjects, mean = intercept_mean, sd = intercept_sd)
...
all_effects <-
  c(intercept_effects, slope_effects) # Put them all together
round(all_effects, 2)
...
jags_data <-
  list(
    ...
    x = as.numeric(x),
    n_subj = max(as.numeric(subjects)),
...
inits <- function() {
  list(
    alpha = rnorm(n_subjects, 0, 2),
    beta = rnorm(n_subjects, 10, 2),
    mu_int = rnorm(1, 0, 1),
    sigma_int = rlnorm(1),
    mu_slope = rnorm(1, 0, 1),
    sigma_slope = rlnorm(1),
    sigma_res = rlnorm(1)
  )
}
...
intercept_slope_correlation <-
  intercept_slope_covariance / (intercept_sd * slope_sd)
...
effects <-
  mvrnorm(n = n_subjects, mu = mu_vector, Sigma = var_covar_matrix)
round(effects, 2)
...
effects_kde <-
  kde2d(effects[, 1], effects[, 2], n = 50) # kernel density estimate
par(mfrow = c(1, 3))
contour(effects_kde)
image(effects_kde)
...
data.frame(
  ...
  intercept_sd = intercept_sd,
  slope_sd = slope_sd,
  intercept_slope_correlation = intercept_slope_covariance / (intercept_sd *
                                                                slope_sd),
  sigma_res = sigma_res
)
...
nt <-
  6
nc <-
  3 # more than this probably needed for a good approx. of the posterior distribution
...
lines(density(out$sims.list$mu_int_prior),
      col = "lightsteelblue3",
      lwd = 2)
hist(
  out$sims.list$mu_int,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
...
lines(density(out$sims.list$mu_int),
      col = "lightsteelblue3",
      lwd = 2)

hist(
  out$sims.list$mu_slope_prior,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
...
lines(density(out$sims.list$mu_slope_prior),
      col = "lightsteelblue3",
      lwd = 2)
hist(
  out$sims.list$mu_slope,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
...
lines(density(out$sims.list$mu_slope),
      col = "lightsteelblue3",
      lwd = 2)

hist(
  out$sims.list$sigma_int_prior,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
  main = "sigma_int_prior"
)
lines(density(out$sims.list$sigma_int_prior),
      col = "lightsteelblue3",
      lwd = 2)
hist(
  out$sims.list$sigma_int,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
...
lines(density(out$sims.list$sigma_int),
      col = "lightsteelblue3",
      lwd = 2)

hist(
  out$sims.list$sigma_slope_prior,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
  main = "sigma_slope_prior"
)
lines(density(out$sims.list$sigma_slope_prior),
      col = "lightsteelblue3",
      lwd = 2)
hist(
  out$sims.list$sigma_slope,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
...
lines(density(out$sims.list$sigma_slope),
      col = "lightsteelblue3",
      lwd = 2)

hist(
  out$sims.list$rho_int_slope_prior,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
  main = "rho_int_slope_prior"
)
lines(density(out$sims.list$rho_int_slope_prior),
      col = "lightsteelblue3",
      lwd = 2)
hist(
  out$sims.list$rho_int_slope,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
...
lines(density(out$sims.list$rho_int_slope),
      col = "lightsteelblue3",
      lwd = 2)
par(mfrow = c(1, 1))
...
(
  intercept_slope1_correlation <-
    intercept_slope1_covariance / (intercept_sd * slope1_sd)
)
intercept_slope2_covariance <- 300
(
  intercept_slope2_correlation <-
    intercept_slope2_covariance / (intercept_sd * slope2_sd)
)
slope1_slope2_covariance <- -200
(slope1_slope2_correlation <-
    slope1_slope2_covariance / (slope1_sd * slope2_sd))
...
(var_covar_matrix <-
    matrix(
      c(
        intercept_sd ^ 2,
        intercept_slope1_covariance,
        intercept_slope2_covariance,
        intercept_slope1_covariance,
        slope1_sd ^ 2,
        slope1_slope2_covariance,
        intercept_slope2_covariance,
        slope1_slope2_covariance,
        slope2_sd ^ 2
      ),
...
data.frame(
  intercept_mean = intercept_mean,
  slope1_mean = slope1_mean,
  slope2_mean = slope2_mean,
  intercept_sd = intercept_sd,
  slope1_sd = slope1_sd,
  slope2_sd = slope2_sd,
  intercept_slope1_covariance = intercept_slope1_covariance,
  intercept_slope2_covariance = intercept_slope2_covariance,
  slope1_slope2_covariance = slope1_slope2_covariance,
  sigma_res = sigma_res
)
...
hist(
  out$sims.list$mu_slope1_prior,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
  main = "mu_slope1_prior"
)
lines(density(out$sims.list$mu_slope1_prior),
      col = "lightsteelblue3",
      lwd = 2)
hist(
  out$sims.list$mu_slope1,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
...
lines(density(out$sims.list$mu_slope1),
      col = "lightsteelblue3",
      lwd = 2)

hist(
  out$sims.list$mu_slope2_prior,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
  main = "mu_slope2_prior"
)
lines(density(out$sims.list$mu_slope2_prior),
      col = "lightsteelblue3",
      lwd = 2)
hist(
  out$sims.list$mu_slope2,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
...
lines(density(out$sims.list$mu_slope2),
      col = "lightsteelblue3",
      lwd = 2)
...
hist(
  out$sims.list$sigma_slope1_prior,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
  main = "sigma_slope1_prior"
)
lines(density(out$sims.list$sigma_slope1_prior),
      col = "lightsteelblue3",
      lwd = 2)
hist(
  out$sims.list$sigma_slope1,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
...
lines(density(out$sims.list$sigma_slope1),
      col = "lightsteelblue3",
      lwd = 2)

hist(
  out$sims.list$sigma_slope2_prior,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
  main = "sigma_slope2_prior"
)
lines(density(out$sims.list$sigma_slope2_prior),
      col = "lightsteelblue3",
      lwd = 2)
hist(
  out$sims.list$sigma_slope2,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
...
lines(density(out$sims.list$sigma_slope2),
      col = "lightsteelblue3",
      lwd = 2)

hist(
  out$sims.list$rho_int_slope1_prior,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
  main = "rho_int_slope1_prior"
)
lines(density(out$sims.list$rho_int_slope1_prior),
      col = "lightsteelblue3",
      lwd = 2)
hist(
  out$sims.list$rho_int_slope1,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
...
lines(density(out$sims.list$rho_int_slope1),
      col = "lightsteelblue3",
      lwd = 2)

hist(
  out$sims.list$rho_int_slope2_prior,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
  main = "rho_int_slope2_prior"
)
lines(density(out$sims.list$rho_int_slope2_prior),
      col = "lightsteelblue3",
      lwd = 2)
hist(
  out$sims.list$rho_int_slope2,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
...
lines(density(out$sims.list$rho_int_slope2),
      col = "lightsteelblue3",
      lwd = 2)

hist(
  out$sims.list$rho_slope1_slope2_prior,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
  main = "rho_slope1_slope2_prior"
)
lines(density(out$sims.list$rho_slope1_slope2_prior),
      col = "lightsteelblue3",
      lwd = 2)
hist(
  out$sims.list$rho_slope1_slope2,
  col = "lightsteelblue1",
  border = "white",
  breaks = 10,
  freq = FALSE,
  main = "rho_slope1_slope2"
)
lines(density(out$sims.list$rho_slope1_slope2),
      col = "lightsteelblue3",
      lwd = 2)
par(mfrow = c(1, 1))
